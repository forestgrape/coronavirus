...
- 31.10. Without correct infection estimated one does blind flight, which can be ok if one is high in the air i.e. low case count but only then. [Detect Monitor Failure](.//monitor_and_control.md#detect-monitor-failure) in the extended section [Monitor Prevalence](./monitor_and_control.md#monitor-prevalence) describes how to check estimates in the past: The current death rate multiplied by inverse fatality should be about the case estimated 4 weeks ago. 

...
## Summary
Since early in the SARS-2 pandemics new variants are observed. Sometimes a competitive advantages against existing variants are observed. Often the competitive advantage is attributed to a higher transmissibility in the sense of a higher average R value. In the following it is argued that a competitive advantage is not necessarily the result of a higher transmissibility but can also result from immune evasion (even at low seroprevalences) or from strains spreading faster. Faster spread alone makes the waves peakier but not larger.

#### Hypotheses
* *Hypothesis 1: Immune Evasion even at low Seroprevalence yields a Competition Advantage*: The Covid spread is highly uneven, in the sense that some individuals have many secondary cases while most individuals have no secondary cases. Thus the distribution of the individual R values is skewed: Many individuals have low R values, while a few have high R values. If a Covid variant now is able to reinfect and spread through people with high R values, it has a competition advantage.
* *Hypothesis 2: Faster Infection yields a Competition Advantage*: Some variants infect faster which makes their incidence rates peakier. These variants have competitions advantages over slower variants even without a higher transmissibility: The fast variants rush through the most hosts with highest R values and leave only few hosts for slow variants, since reinfection within a couple of weeks is rare.


#### Epidemiological Evidences
The hypotheses above explain patterns observed for the spread of new variants: fast strains such as alpha and delta out-compete other variants especially when many new hosts become available i.e. in spread-favorable situation such as the release of measures or spread-friendly climate situations and immune evasion variants are often observed when there's preexisting immunity e.g. the beta variant in northern Brazil.

#### Biological Explanations
Some biological factors underlying the above effects are sketched. 
* Neutralizing antibody escape can arise through points mutations in the S1  domain of the spike
* Mutations affecting the cleavage sites can increase the priming through proteases which yields a shorter virion time which results in faster infections (and also effectiveness of antibodies due to a shorter exposure).
...
Regarding SARS-CoV-2, new lineages emerge and often show a competition advantage. The underlying mechanisms for the competition advantages are often unclear. Often from the competition advantage a higher overall R values is presumed. The goal of this chapter is to outline that a competition advantage can have many causes:
...
> 2. and 3. can yield higher transmissibility i.e. an increase of the average R value. If this is the case the effects of 2. can be reduced by information. The effects of 3. by testing. 
> 
> It is also possible that the average R value of faster spreading variants is not higher since faster infecting viruses may yield more localized infections which the immune system can better prevent.


### Biological Explanations
#### Faster Spike Priming
...
* the existing proteases can better prime
* new proteases can prime the spikes.

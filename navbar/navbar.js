...
function onContentMove() {
    ...
    if (side_nav.style.display != "none") {
        ...
        //no guarding with if, since it possibly prevents up scrolling on jumps (e.g. link)
    
        side_nav.scrollTo({ top: loc_of_active<=lowest_placement?0:loc_of_active - lowest_placement, 'behavior': 'smooth' }) //<=> side_nav.scrollTo({ top: Math.max(0, loc_of_active - lowest_placement), 'behavior': 'smooth' })
